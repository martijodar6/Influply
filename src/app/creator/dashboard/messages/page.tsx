...
import { listConversationsForCreator } from '@/lib/queries';

export default async function CreatorMessagesPage() {
  const { user, profile } = await requireCreatorProfile();
  const conversations = await listConversationsForCreator(profile.id);
  ...
  return (
    <DashboardShell items={creatorNavItems} activePath="/creator/dashboard/messages" userName={user.name ?? ''} role={user.role}>
      ...
      <p className="mb-6 text-sm text-ink-500">Conversaciones con empresas.</p>

      {conversations.length === 0 ? (
        <EmptyState
          title="Aún no tienes conversaciones"
          description="Escribe a una empresa desde su perfil para empezar a chatear."
        />
      ) : (
        <div className="flex flex-col gap-2">
          {conversations.map((c) => (
            <Link
              key={c.id}
              href={`/creator/dashboard/messages/${c.id}`}
